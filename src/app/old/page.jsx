...
function MainComponent() {
  ...
  const handleDragStartUnscheduled = (e, index) => {
    if (e.type === "touchstart") {
      setTouchStartY(e.touches[0].clientY);
      setDraggedItem(index);
      setDragSource("unscheduled");
      return;
    }
    const originalRect = e.target.getBoundingClientRect();
    const dragPreview = e.target.cloneNode(true);
  ...
  const handleDragEnd = (e) => {
    if (e.type === "touchend") {
      ...
      const droppable = elements.find((el) =>
        el.getAttribute("data-droppable")
      );
      if (droppable) {
        const index = parseInt(droppable.getAttribute("data-index"));
        if (!isNaN(index)) {
          handleDrop(e, index);
        }
      }
  ...
  return (
    <>
      ...
      >
        {notification && (
          <div className="fixed bottom-4 left-4 bg-gray-800 border border-gray-700 text-gray-100 px-4 py-2 rounded-md flex items-center gap-2 shadow-md notification-fade">
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full max-w-6xl">
          <div className="w-full sm:w-[calc(50%-25px)] bg-white rounded-lg shadow-md overflow-hidden min-h-[150px] sm:min-h-[600px] flex flex-col">
            ...
            <div
              className="flex-1 p-2 min-h-[200px] bg-gray-50"
              onDragOver={(e) => {
                e.preventDefault();
                setDragOverIndex(null);
              }}
            ...
            >
              {unscheduledItems.map((item, index) => (
                <div
                  key={item.id}
                  draggable
                  onDragStart={(e) => handleDragStartUnscheduled(e, index)}
                  onDragEnd={handleDragEnd}
                  className={`bg-white mb-2 last:mb-0 rounded-md shadow-sm hover:shadow-md transition-shadow group cursor-grab active:cursor-grabbing border-l-4 ${getBorderColor(
                    item.duration
                  )}`}
                >
                  <div
                    className={`flex items-center justify-between px-4 py-3 ${
                      draggedItem === index && dragSource === "unscheduled"
                        ? "invisible"
                        : ""
                    }`}
                  >
          ...
          <div className="w-full sm:w-[calc(50%+25px)] bg-white rounded-lg shadow-md overflow-hidden min-h-[600px] flex flex-col">
            <div className="p-4 border-b">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <h2 className="text-gray-800 font-roboto text-lg">
                    Schedule
                  </h2>
                  <button
            ...
            <div
              className="flex-1 min-h-[400px] bg-gray-50 pb-2 p-2"
              onDragOver={(e) => {
                e.preventDefault();
                  ...
                  <div key={item.id} className="flex items-start w-full">
                    ...
                    <div className="flex-1">
                      ...
                      <div
                        ...
                        className={`mt-2 bg-white rounded-md shadow-sm hover:shadow-md transition-shadow group cursor-grab active:cursor-grabbing border-l-4 ${getBorderColor(
                          item.duration
                        )}`}
                      >
                        <div
